...
export class WorkoutSessionController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Free workout session created successfully.',
  })
  ...
  @ApiResponse({
    status: 201,
    description: 'Workout session created from template successfully.',
  })
  ...
  @ApiResponse({
    status: 200,
    description:
      'Active workout session retrieved successfully, or null if none.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async getActive(@CurrentUser() user) {
    return this.service.findActiveWorkout(user.id);
  }
  ...
  @ApiResponse({ status: 404, description: 'Session not found.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async delete(@CurrentUser() user, @Param('sessionId') sessionId: string) {
  ...
  @ApiResponse({ status: 404, description: 'Exercise not found.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async deleteExercise(@CurrentUser() user, @Param('id') id: string) {
  ...
  @ApiResponse({ status: 404, description: 'Set not found.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async deleteSet(@CurrentUser() user, @Param('id') id: string) {
